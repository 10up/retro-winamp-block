...
1. Version bump: Bump the version number in `package-lock.json`, `package.json`, `readme.txt`, and `retro-winamp-block.php` if it does not already reflect the version being released.
...
1. SVN: Wait for the [GitHub Action](https://github.com/10up/retro-winamp-block/actions/workflows/wordpress-plugin-deploy.yml) to finish deploying to the WordPress.org repository. If all goes well, users with SVN commit access for that plugin will receive an emailed diff of changes.
